...
gulp.task('iconfont', function (done) {
	const iconStream = gulp.src([`${dist}/app/themes/pro/images/svg/**/*`])
		.pipe(iconfont({
                fontName: `iconfont-pro`,
                normalize: true,
                fontHeight: 1001,
	...
	async.parallel([
            function handleGlyphs(cb) {
				...
				async.parallel([
						...
						gulp.src(`${dist}/app/themes/pro/css/iconfont/_iconfont.{sass,scss}`)
						.pipe(consolidate('lodash', {
                                            glyphs,
							fontCacheBuster: Math.random().toString(36).substring(7),
                                            fontName: `iconfont-pro`,
                                            themePath: `/themes/pro/`,
                                            fontPath: `fonts/iconfont-pro`,
							className: 'icon'
						}))
						.pipe(gulp.dest(`web/app/themes/pro/css/__iconfont`))
                                    .on('finish', cb);
                            },
                            function iconfontVariables(cb) {
						gulp.src(`${dist}/app/themes/pro/css/iconfont/_iconfont-variables.{sass,scss}`)
							.pipe(consolidate('lodash', {
                                            glyphs,
								fontCacheBuster: Math.random().toString(36).substring(7),
                                            fontName: `iconfont-pro`,
							...
							.pipe(gulp.dest(`${dist}/app/themes/pro/css/__iconfont`))
							.on('finish', cb)
					}
				], done)
                });
            ...
            function handleFonts(cb) {
                iconStream
				.pipe(gulp.dest((`${dist}/app/themes/pro/fonts/iconfont-pro`)))
                    .on('finish', cb);
		}
	], done);
});
