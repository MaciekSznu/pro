...
function get_blog_heading() {
	if ( is_day() ) {
		return __( 'Daily Archives:', 'pro' ) . get_the_date();
	} elseif ( is_month() ) {
		return __( 'Monthly Archives:', 'pro' ) . get_the_date( _x( 'F Y', 'monthly archives date format', 'pro' ) );
	} elseif ( is_year() ) {
		return __( 'Yearly Archives:', 'pro' ) . get_the_date( _x( 'Y', 'yearly archives date format', 'pro' ) );
	} elseif ( is_category() ) {
		return __( 'Category:', 'pro' ) . ' ' . single_cat_title( '', false );
	} elseif ( is_author() ) {
		$author_data = get_query_var( 'author_name' ) ? get_user_by( 'slug', get_query_var( 'author_name' ) ) : get_userdata( get_query_var( 'author' ) );
		return __( 'Author:', 'pro' ) . ' ' . $author_data->display_name;
	} elseif ( is_tag() ) {
		return __( 'Tag:', 'pro' ) . ' ' . single_tag_title( '', false );
	} elseif ( is_search() ) {
		return __( 'Search:', 'pro' ) . ' ' . get_search_query();
	} else {
		return __( 'Blog Archives', 'pro' );
	}
	return null;
}
